...
export async function generateMetadata({ params }) {
  ...
  const currentPath = `/${categorySlug}/${productSlug}`;
  ...
  // Enforce Primary Category canonical matching
  const canonicalCatSlugRaw = getProductPrimaryCategorySlug(product);
  const canonicalCatSlug = canonicalCatSlugRaw === 'uncategorized' ? 'shop' : canonicalCatSlugRaw;

  if (canonicalCatSlug !== categorySlug) {
    permanentRedirect(`/${canonicalCatSlug}/${productSlug}`);
  }
...
export default async function NestedProductDetailPage({ params }) {
  const { slug: categorySlug, productSlug } = await params;
  ...
  return (
    <div className="bg-white min-h-screen font-sans">
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-16 py-4 md:py-8">
        {/* Rebuilt Breadcrumb System matching requirement 6 */}
        <nav className="flex items-center gap-2 mb-6 text-[10px] md:text-[11px] font-medium uppercase tracking-wider text-primary/60 border-b border-black/5 pb-4 overflow-x-auto whitespace-nowrap scrollbar-hide">
          ...
          {primaryCategory ? (
            <>
              <Link href={`/${primaryCategory.slug}`} className="hover:text-primary transition-colors shrink-0">{primaryCategory.name}</Link>
              <ChevronRight className="w-2.5 h-2.5 opacity-40 shrink-0" />
            </>
          ) : (
            <>
